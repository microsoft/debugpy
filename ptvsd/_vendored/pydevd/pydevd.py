...
class PyDB(object):
    ...
    def do_wait_suspend(self, thread, frame, event, arg, suspend_type="trace", is_unhandled_exception=False):  # @UnusedVariable
    ...
    def _do_wait_suspend(self, thread, frame, event, arg, suspend_type, from_this_thread):
        ...
        if info.pydev_step_cmd == CMD_STEP_INTO or info.pydev_step_cmd == CMD_STEP_INTO_MY_CODE:
            ...
            if stop:
                ...
                self._do_wait_suspend(thread, frame, event, arg, suspend_type, send_suspend_message=False)
                return
